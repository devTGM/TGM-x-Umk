class FacetFiltersForm extends HTMLElement {
  ...
  static renderPage(e, t, r = !0) {
    FacetFiltersForm.searchParamsPrev = e;
    const n = FacetFiltersForm.getSections(),
      o = document.getElementById("ProductCount"),
      a = document.getElementById("ProductCountDesktop");
    (document
      .getElementById("ProductGridContainer")
      .querySelector(".collection-grid-container")
      .classList.add("loading"),
      o && o.classList.add("loading"),
      a && a.classList.add("loading"),
      n.forEach((r) => {
        const n = `${window.location.pathname}?section_id=${r.section}&${e}`,
          o = (e) => e.url === n;
        FacetFiltersForm.filterData.some(o)
          ? FacetFiltersForm.renderSectionFromCache(o, t)
          : FacetFiltersForm.renderSectionFromFetch(n, t);
      }),
      r && FacetFiltersForm.updateURLHash(e));
  }
    ...
    fetch(e)
      ...
      .then((r) => {
        ...
        ((FacetFiltersForm.filterData = [
          ...
          FacetFiltersForm.renderFilters(n, t),
          FacetFiltersForm.renderProductGridContainer(n),
          FacetFiltersForm.renderProductCount(n),
          FacetFiltersForm.applyInstantShipFilter(),
  ...
  static renderSectionFromCache(e, t) {
    const r = FacetFiltersForm.filterData.find(e).html;
    (FacetFiltersForm.renderFilters(r, t),
      ...
      document.getElementById("SortBy") &&
        document
          ...
          .dispatchEvent(new Event("change", { bubbles: !0 })),
      "function" == typeof initializeScrollAnimationTrigger &&
        initializeScrollAnimationTrigger(r.innerHTML));
  }
  ...
  static renderFilters(e, t) {
    const r = new DOMParser().parseFromString(e, "text/html"),
      n = r.querySelectorAll("#FacetFiltersForm .js-filter"),
      o = document.querySelectorAll("#FacetFiltersForm .js-filter");
    Array.from(o).forEach((e) => {
      Array.from(n).some(({ id: t }) => e.id === t) || e.remove();
    });
    const a = (e) => {
        const r = t ? t.target.closest(".js-filter") : void 0;
        return !!r && e.id === r.id;
      },
      i = Array.from(n).filter((e) => !a(e)),
      c = Array.from(n).find(a);
    if (
      (i.forEach((e, t) => {
        if (document.getElementById(e.id))
          document.getElementById(e.id).innerHTML = e.innerHTML;
        else if (t > 0) {
          const { className: r, id: n } = i[t - 1];
          if (e.className === r)
            return void document.getElementById(n).after(e);
        }
      }),
      FacetFiltersForm.renderActiveFacets(r),
      c)
    ) {
      const e = t.target.closest(".js-filter").id;
      if (e) {
        FacetFiltersForm.renderCounts(c, t.target.closest(".js-filter"));
        const r = document.getElementById(e).querySelector(".facets__summary"),
          n = "text" === t.target.getAttribute("type");
        r && !n && r.focus();
      }
    }
  }
  static renderActiveFacets(e) {
    ([".active-facets"].forEach((t) => {
      const r = e.querySelector(t);
      r && (document.querySelector(t).innerHTML = r.innerHTML);
    }),
      FacetFiltersForm.toggleActiveFacets(!1));
    FacetFiltersForm.bindRemoveInstantShip();
  }
  static bindRemoveInstantShip() {
    document.querySelectorAll(".js-remove-instant-ship").forEach((el) => {
      if (!el._bound) {
        el.addEventListener("click", (e) => {
          e.preventDefault();
          document.querySelectorAll(".js-instant-ship-checkbox").forEach((cb) => {
            cb.checked = false;
          });
          const form = document.querySelector("facet-filters-form");
          if (form) {
            form.onSubmitHandler(e);
          }
        });
        el._bound = true;
      }
  ...
  static applyInstantShipFilter() {
    const searchParams = new URLSearchParams(window.location.search);
    const isInstantShipActive =
      searchParams.has("filter.p.m.custom.instant_ship") ||
      searchParams.has("instant_ship") ||
      Array.from(document.querySelectorAll(".js-instant-ship-checkbox")).some(
        (cb) => cb.checked,
      );

    // Sync all instant ship checkboxes
    document.querySelectorAll(".js-instant-ship-checkbox").forEach((cb) => {
      cb.checked = isInstantShipActive;
      cb.closest(".facets__item")?.classList.toggle("facets__item--checked", isInstantShipActive);
    });
    ...
    if (productCards && productCards.length > 0) {
      ...
      productCards.forEach((card) => {
        if (isInstantShipActive) {
          const isInstant =
            card.hasAttribute("data-instant-ship") ||
            card.querySelector(".badge.instant-ship");
          if (isInstant) {
            card.style.display = "";
            visibleCount++;
          } else {
  ...
  static renderCounts(e, t) {
    ...
    a && o && (o.outerHTML = a.outerHTML);
    const i = t.querySelector(".facets-wrap"),
  ...
  onSubmitHandler(e) {
    ...
    const t = new URLSearchParams(),
      r = new URL(window.location.href).searchParams.get("sort_by");
    r && t.set("sort_by", r);
    const n = document.getElementById("SortBy");
    n && n.value && t.set("sort_by", n.value);
    (document.querySelectorAll("facet-filters-form form").forEach((e) => {
...
((FacetFiltersForm.filterData = []),
  ...
  (window.FacetFiltersForm = FacetFiltersForm),
...
class PriceRange extends HTMLElement {
  ...
  setMinAndMaxValues() {
    const e = this.querySelectorAll("input"),
      ...
      r = e[1];
    (r.value
      ? t.setAttribute("data-max", r.value)
      : t.setAttribute("data-max", r.getAttribute("max")),
      t.value
        ? r.setAttribute("data-min", t.value)
        : r.setAttribute("data-min", t.getAttribute("min")));
  }
  adjustToValidValues(e) {
    const t = Number(e.value),
      r = Number(e.getAttribute("min")) || 0,
      n = Number(e.getAttribute("max"));
    (!isNaN(r) && t < r && (e.value = r), !isNaN(n) && t > n && (e.value = n));
  }
}
